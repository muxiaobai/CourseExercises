...
predict_cons = ['date','day_of_week','brand']
...
predicted_test_prices = yancheng_model.predict(test_data[predict_cons])
#print (predicted_home_prices)
int_cnt = np.around(predicted_test_prices)

my_submission = pd.DataFrame({'date':test_data.date,,'brand': test_data.brand,'cnt':int_cnt.astype(int)})
my_submission.to_csv('submissionB20180207.csv',index = False,header = False,columns = ['date','cnt'])
my_submission.to_csv('resultB20180207.txt',index=False,header=False,columns = ['date','cnt'],sep='\t')
