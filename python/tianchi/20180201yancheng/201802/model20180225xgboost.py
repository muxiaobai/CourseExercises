...
print mean_squared_error(val_y,predicted)


# predict and save output
#print ("The predictions are")
predicted_test_prices = (xgb.predict(test[predict_cons]) + gbdt.predict(test[predict_cons]))/2
int_cnt = np.around(predicted_test_prices)

my_submission = pd.DataFrame({'date':test.date,'cnt':int_cnt.astype(int)})
my_submission.to_csv('submission20180225.csv',index = False,header = False,columns = ['date','cnt'])
my_submission.to_csv('result20180225.txt',index=False,header=False,columns = ['date','cnt'],sep='\t')
